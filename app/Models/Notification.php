...
class Notification extends Model
{
    ...
    public static function createNewBid($userId, $bidData = []): self
    {
        ...
        $brandName = $bidData['brand_name'] ?? 'Unknown Brand';
        ...
        return self::create([
            ...
            'message' => "{$creatorName} submitted a bid of R$ {$bidAmount} for {$brandName}'s campaign: {$campaignTitle}",
            'data' => $bidData,
        ]);
    }
    ...
    public static function createPaymentActivity($userId, $paymentData = []): self
    {
        ...
        $message = "Payment activity: {$userName} - {$paymentType}";
        if ($amount) {
            $message .= " (R$ {$amount})";
        }
